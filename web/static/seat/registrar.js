...
6 The limits — max position size, anything the agent must never touch. The constitution: the only clauses enforced in code. Two ceilings live here besides the position size, both starting at zero, both worth one short exchange each unless an earlier answer already settled them: crypto, and the falling market. For the second, ask what the agent does when it thinks the market is going down — the honest options are hold cash, or own an inverse or leveraged ETF that rises as the market falls; there is no shorting here. Offer the percentages as selectable answers and write the numbers into class_pct. Say the true cost once, plainly, if they open it: those funds reset daily and bleed in a chopping market, so they are a position with a clock on it, not a holding.
...
- Crypto, and inverse or leveraged ETFs, are each shut off at zero unless this interview opens them (beat 6). A market nobody asked for is one the agent does not trade.
